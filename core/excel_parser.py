"""
UniSync — Excel Parser + Full Seed Data
Each routine entry now includes course_year and course_semester
so the system can filter by the logged-in user's exact semester.
"""
import pandas as pd
import re
from typing import List, Dict, Optional

# ── Course → (program, year, semester) mapping ────────────────
COURSE_META = {
    ...
    'GED-2104': ('BBA', 2, 2),
    'MGT-2105': ('BBA', 2, 2),
    # BBA Year 3 Semester 1
    'MGT-3101': ('BBA', 3, 1),
    'MGT-3102': ('BBA', 3, 1),
    'MGT-3103': ('BBA', 3, 1),
    'MGT-3104': ('BBA', 3, 1),
    'MGT-3105': ('BBA', 3, 1),
    # BBA Year 3 Semester 2
    'MGT-3201': ('BBA', 3, 2),
    'MGT-3202': ('BBA', 3, 2),
    'MGT-3203': ('BBA', 3, 2),
    'MGT-3204': ('BBA', 3, 2),
...
def parse_cell(cell_value: str) -> Optional[Dict]:
    if not cell_value or str(cell_value).strip() in ['', 'nan', 'NaN']:
        return None
    match = re.match(r'^([A-Z]+)\s*\(([A-Z]+-\d+)\)$', str(cell_value).strip())
    if match:
        return {'teacher_code': match.group(1), 'course_code': match.group(2)}
    return None
