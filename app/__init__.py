...
def create_app(config_name: str = None):
    ...
    def email_check():
        ...
        config_info = {
            ...
            'MAIL_FROM':       mail_from or '⚠️ not set (using default: onboarding@resend.dev)',
        }
        ...
        return jsonify({
            'config':     config_info,
            'connection': conn,
            'next':       '/api/test-email?to=your@email.com' if conn['ok'] else 'Fix errors first',
        }), 200 if conn['ok'] else 500

    # ════════════════════════════════════════════════════════
    # STEP 2: Real email পাঠাও, full error দেখাও
        ...
        if not to:
            return jsonify({
                'usage': {
                    ...
                    'send':  '/api/test-email?to=your@email.com',
                }
            }), 400
